...
export default {
  ...
  async loginUser(data) {
    ...
    const hashedPassword = hash.hashPassword(data.password);
    const comparePassword = hash.compareHashedPassword(data.password, hashedPassword);
    if(!comparePassword) throw createError('dados incorretos.', 400);
    const userEncontrado = data
    const payload = {userId: userEncontrado._id, email: userEncontrado.email, role: userEncontrado.role};
